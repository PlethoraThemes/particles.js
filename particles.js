...
var particlesJS = (function(){
  ...
  /*** PlethoraThemes | Additional Functions ***/

  var particlesInit = function particlesJS(el, params){
    ...
    /* append canvas */
    // var canvas = document.getElementById(el).appendChild(canvas_el);
    var canvas = el.appendChild(canvas_el);

    /* launch particle.js */
    if( canvas !== null ){
      launchParticlesJS( el, params );
    }

  };

  /*
   * Find all elements with a class "particles-js" and initialize ParticlesJS on them.
   */
  return function() {

    var particleContainer = document.getElementsByClassName("particles-js");
    ...
    for ( var i = 0, len = particleContainer.length; i < len; i++ ) {
      ...
      particleContainer[i].insertBefore( particleCanvasContainer, particleContainer[i].firstChild );
      particlesInit( particleCanvasContainer );
...
if ( window.addEventListener ) { 
  ...
  window.addEventListener( "load", particlesJS, false ); 

} else if ( window.attachEvent ) { 

  window.attachEvent( "onload", particlesJS ); 
